...
function _parseLink3ProfileWithKey (data, key) {
  ...
  return { did: 'link3', key, name, avatar, bio, socials }
}
...
export async function getProfileFromKey (key) {
  ...
  const link3Profile = await gqlClient.request(getLink3ProfileData, { handle: key })
  // if not found, will return { profileByHandle: null }
  if (!link3Profile.profileByHandle) {
    return
  }

  return { addr, ..._parseLink3ProfileWithKey(link3Profile, key) }
}
