...
export async function getProfileFromHandle(handle) {
  ...
  const avatar = records.find(item => item.key === 'profile.avatar')?.value
  const bio = records.find(item => item.key === 'profile.description')?.value

  // the key of item will be like: prifile.twitter
...
export async function getProfileFromAddress(addr) {
  ...
  return await getProfileFromHandle(handle)
}
