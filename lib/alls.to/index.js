...
export async function queryWithAddr (addr, click) {
  const type = /^sui:0x[0-9a-f]{64}$/.test(addr) ? 'sui' : getAddressFormat(addr)
  if (!type) {
    return
  }
  ...
  if (click) {
    // TODO
    const match = await AllsTo.findOneAndUpdate({ addr }, { $inc: { clicks: 1 } }, { new: true }).sort({ did: -1 })
    toList = match ? [match] : []
  } else {
  ...
  let profile
  if (type === 'ethers') {
    profile = await getProfileFromAddress(addr, 'cyber')
  }
...
async function _queryWithHandleOrKey (handleOrKey, click) {
  ...
  if (click) {
    // TODO
    const match = await AllsTo.findOneAndUpdate({ $or: conditions }, { $inc: { clicks: 1 } }, { new: true }).sort({ did: -1 })
    return match ? [match] : []
  } else {
...
export async function queryPage (handleOrKeyOrAddr, click) {
  ...
  // TODO: @yangyi describe what this part is doing using plain English
  const profile = await getProfileFromHandleOrKey(handleOrKey, 'cyber')
  if (!profile) {
    return []
  }
