...
import { DIDs, getProfileFromAddress, getProfileFromKey, getProfileFromHandle } from 'lib/did'
...
export async function queryPage(handleOrKeyOrAddr, click) {
  // @yangyi describe what this part is doing using plain English
  ...
  // @yangyi describe what this part is doing using plain English
  const profile = await getProfileFromHandle(handleOrKey)
  if (!profile) {
    return []
  }

  const { key, ...rest } = profile
